...
| `/public-view/profilo` | Public | PublicLayout | Profilo standalone (dati personali + arricchimento Pipeline + sezione `Comunicazioni` = consenso commerciale). Il consenso va all'identità risolta: Pipeline → `marketing_consents[emailVerificata]` (tri-state: chiave assente = sconosciuto); Student risolto → `Student.marketing_consent`, nessuna Pipeline creata. Termini/Privacy stanno su `/public-view/account`. Nel sidebar è nello slot secondario in basso. Cross-link a `/public-view/account` |
