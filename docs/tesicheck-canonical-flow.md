...
> **Stato prototipo (non canonico).** Parziale. Esistono ora: il modello persistente path-bound dei check Coach (`CoachPathBoundCheck`), lo store dedicato `coach-tesicheck-checks-v1`, la route report nella shell Coach `/coach-view/report/:checkId`, la materializzazione di un singolo check path-bound che apre il report Coach, e lo Storico Coach (`/coach-view/history`, `/coach-view/archivio`) collegato allo store: legge `coach-tesicheck-checks-v1` filtrando per owner Coach, mostra il contesto path-bound (studente, percorso, crediti usati dal singolo check), deriva la disponibilità `Completato` / `Scaduto` da `expiresAt` a render time e apre il report Coach. **Non** esiste ancora la modalità `Check libero`, quindi lo Storico contiene solo record `binding.mode === 'coaching_path'`. L'entitlement/quota resta logica prototipo/mock. Lo Storico Coach non riusa lo store persistente consumer (`public-tesicheck-checks-v1`). Dettaglio tecnico: [tesicheck-coach-handoff.md](./tesicheck-coach-handoff.md).
