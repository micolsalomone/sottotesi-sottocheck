...
// ─── Types ───────────────────────────────────────────────────
export type ServiceStatus = 'pending_payment' | 'active' | 'paused' | 'completed' | 'cancelled' | 'expired';
...
// ─── Pipeline types ──────────────────────────────────────────
export type QuoteStatus = 'draft' | 'sent' | 'accepted' | 'rejected' | 'expired' | 'expiring_soon';
...
const initialData: StudentService[] = [
  ...
  },
  {
    ...
    installments: [
      {
        id: 'INS-501', amount: 99, dueDate: '2026-02-10', status: 'pending'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    needs_timeline: true, // Prevista ma da creare (coaching pending_payment, timeline da onboardare)
  },
  ...
  { id: 'SS-157', pipeline_id: 'PIP-030', student_id: 'STU-590', student_name: 'Beatrice Vitale', service_id: 'SRV-001', service_name: 'Starter Pack', service_category: 'Starter Pack', quote_id: 'QT-975', status: 'pending_payment', created_at: '2026-02-20', created_by: 'Francesca', updated_at: '2026-02-20', updated_by: 'Francesca', referente: 'Giada', plan_start_date: '2026-03-01', plan_end_date: '2026-04-01', contract: { id: 'CT-157', status: 'draft', documentUrl: '/contratti/CT-157.pdf' }, installments: [{ id: 'INS-1090', amount: 99, dueDate: '2026-03-01', status: 'pending' }] },
  ...
  { id: 'SS-167', pipeline_id: 'PIP-031', student_id: 'STU-596', student_name: 'Simone Caruso', service_id: 'SRV-001', service_name: 'Starter Pack', service_category: 'Starter Pack', quote_id: 'QT-992', status: 'pending_payment', created_at: '2026-03-05', created_by: 'Claudia', updated_at: '2026-03-05', updated_by: 'Claudia', referente: 'Giada', plan_start_date: '2026-03-15', plan_end_date: '2026-04-15', contract: { id: 'CT-167', status: 'draft', documentUrl: '/contratti/CT-167.pdf' }, installments: [{ id: 'INS-1140', amount: 99, dueDate: '2026-03-15', status: 'pending' }] },
];
...
export function LavorazioniProvider({ children }: { children: React.ReactNode }) {
  ...
  // Auto-check for expired quotes on mount
  React.useEffect(() => {
    const now = new Date();
    // Normalize today to start of day for cleaner comparison with date inputs
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const fiveDaysFromNow = new Date(today);
    fiveDaysFromNow.setDate(today.getDate() + 5);

    setPipelines(prev => prev.map(p => {
      if (!p.quotes || p.quotes.length === 0) return p;
      let hasChanges = false;
      const updatedQuotes = p.quotes.map(q => {
        if (q.expires_at && q.status !== 'accepted' && q.status !== 'rejected') {
          ...
          if (today >= expiryDate) {
            ...
            if (q.status !== 'expiring_soon' && q.status !== 'expired') {
              hasChanges = true;
              return { ...q, status: 'expiring_soon' as QuoteStatus };
            }
          }
        }
        return q;
      });
      return hasChanges ? { ...p, quotes: updatedQuotes } : p;
    }));
  }, []);
