 ...
 * - this module is ACQUISITION/CRM-only. `/public-view/profilo`
 *   (`PublicProfilePage`) and the post-payment academic review do NOT use it —
 *   they read/write a separate, CRM-free prototype store
 *   (`src/app/data/standaloneProfile.ts`) so the public-facing Profile/review
 *   never depends on Pipeline-vs-Student resolution. `ensureTesiCheckPipeline`
 *   / `applyStandaloneRegistrationConsent` still run at registration exactly as
 *   before; `seedStandaloneProfileFromRegistration` (in `standaloneProfile.ts`)
 *   additionally mirrors that SAME explicit choice onto the Profile store, in
 *   parallel — this module never reads that store back.
 */
